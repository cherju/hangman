...
def get_word():  # фунция получения рандомного слова
...
def play(word):
    ...
    while tries > 0:
        ...
        if guess in guessed_letters or guess in guessed_words and guess != word:
            print('Вы уже это предполагали')
            continue
        if len(guess) == 1:
        ...
        if 1 < len(guess) < len(word):
            print('Либо 1 буква, либо слово целиком')
            continue
        ...
        if guess in word:
            ...
            for i in range(len(word)):
                if guess == word[i]:
                    ...
                    a.append(word_completion[i])
            word_completion = ''.join(a)
        print(word_completion)
        if guess == word or word_completion == word:
            print(word, 'Вы угадали! Поздравляю!')
            break
        else:
    ...
    if tries == 0 and guess != word:
        print('Вы проиграли. Загаданное слово было', word)


if input('Сыграем в слова? Д / Н') == 'Д':
    word = get_word()
    play(word)
